...
export const CreditsStatusBanner: React.FC = () => {
  ...
  const problems: Array<{ provider: string; s: ProviderStatus }> =
  ...
  return (
    <div
      className={`rounded-2xl border p-5 space-y-3 ${allBroken ? "border-red-500/50 bg-red-500/10" : "border-amber-500/50 bg-amber-500/10"}`}
    >
      {problems.map(({ provider, s }) => {
        const copy = s.reason ? REASON_COPY[s.reason] : null;
        const severityColor =
          copy?.severity === "error" ? "text-red-400" : "text-amber-400";
        return (
          <div key={provider} className="flex items-start gap-3">
            <AlertTriangle
              className={`w-5 h-5 ${severityColor} mt-0.5 shrink-0`}
            />
            <div className="flex-1">
              ...
              {copy && (
                <a
                  href={copy.actionUrl}
                  target={
                    copy.actionUrl.startsWith("http") ? "_blank" : undefined
                  }
                  rel="noopener"
