  ...
  const loadDirectory = async (path: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/browse-files?path=${encodeURIComponent(path)}`,
        {
      ...
      <div className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 overflow-y-auto min-h-[300px] max-h-[500px]">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-zinc-500 text-sm">
              Folder is empty or contains no supported videos
            </div>
          ) : (
            <div className="py-1">
              {entries.map((entry) => (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1 min-w-0">
                    <span
                      className={`text-sm font-medium truncate transition-colors ${entry.is_dir ? "text-zinc-300 group-hover:text-white" : "text-white"}`}
                    >
                      {entry.name}
                    </span>
                  </div>
        ...
        <div className="px-5 py-4 border-t border-white/5 bg-black/20 rounded-b-2xl">
          <p className="text-xs text-zinc-500 text-center">
            Select a .mp4, .mov, or .mkv file directly from your hard drive to
            process it instantly without uploading.
          </p>
        </div>
      </div>
    </div>
