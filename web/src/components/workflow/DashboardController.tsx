...
export const DashboardController: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const check = async () => {
            try {
                ...
                const running = history.find((j: any) =>
                    j.status === 'processing' || j.status === 'pending' || j.status === 'resuming' || j.status === 'paused'
                );
                if (!cancelled) {
                    setOrphanJob(running ? { id: running.id, message: running.message || `Processing ${running.filename}...` } : null);
                }
            } catch { /* ignore */ }
        };
    ...
    const handleResumeEpisode = async (episodeNum: number) => {
        guardWithApiKey(async () => {
            ...
            try {
                ...
                toast.error(error.message || 'No se pudo reanudar', { id: loadingToast });
            }
        });
    };
    ...
    const handleResetEpisode = async (episodeNum: number) => {
        guardWithApiKey(async () => {
            ...
            try {
                const response = await ClipsApi.processEpisode(episodeNum, { ...getPipelineDefaults(), force_reset: true });
        ...
        <div className="flex flex-col gap-12">
            ...
            {activeJobId && (
                <LiveProcessingWidget
                    ...
                    onCancel={async () => {
                        try { await ClipsApi.cancelJob(activeJobId); } catch (_) {}
                        clearActiveJob();
                    }}
                    onPause={async () => {
                        try { await ClipsApi.pauseJob(activeJobId); } catch (_) {}
                    }}
                    onResume={async () => {
                        try { await ClipsApi.resumeJob(activeJobId); } catch (_) {}
                    }}
                />
            )}
            ...
            {!activeJobId && orphanJob && (
                <div className="flex items-center justify-between gap-4 px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-white">Procesamiento en curso</p>
            ...
            <LibraryView onProcessEpisode={handleProcessEpisode} onResumeEpisode={handleResumeEpisode} onResetEpisode={handleResetEpisode} />
