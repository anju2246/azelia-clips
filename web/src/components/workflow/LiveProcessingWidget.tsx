        ...
        ws.onmessage = (event) => {
            try {
                ...
                if (msg.event === 'progress') {
                    ...
                    setJob(prev => prev ? { ...prev, status: 'error', error: msg.data.error } : null);
                    toast.error(`Processing error: ${msg.data.error}`);
                } else if (msg.event === 'completed') {
                    setJob(prev => prev ? { ...prev, status: 'completed', progress: 100, message: 'Done!' } : null);
    ...
    return (
        <div className="w-full max-w-2xl mx-auto mt-10">
            <div className="bg-zinc-900/40 backdrop-blur-xl border border-white/10 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
                ...
                {isCompleted && <div className="absolute top-0 left-0 h-1 w-full bg-green-500" />}
                {isError && <div className="absolute top-0 left-0 h-1 w-full bg-red-500" />}
                ...
                <div className="flex items-start justify-between mb-8">
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${wsStatus === 'connected' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                                wsStatus === 'connecting' ? 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20' :
                ...
                <div className="mb-8">
                    ...
                    <div className="h-2 w-full bg-black rounded-full overflow-hidden border border-white/5">
                        <div
                            className={`h-full transition-all duration-300 ease-out ${isError ? 'bg-red-500' : isCompleted ? 'bg-green-500' : 'bg-brand-500'
                                }`}
                            style={{ width: `${progressPercent}%` }}
                        />
                    </div>
                ...
                {isError && job.error && (
                    <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl mb-6">
                        ...
                        <p className="text-red-300/80 text-sm font-mono whitespace-pre-wrap">{job.error}</p>
                    </div>
                )}
