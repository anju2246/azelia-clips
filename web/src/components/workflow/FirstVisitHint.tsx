 ...
 */
export const FirstVisitHint: React.FC<{ onScrollToUpload?: () => void }> = ({
  onScrollToUpload,
}) => {
  ...
  const dismiss = async (markServer: boolean) => {
    ...
    try {
      ...
      const key = (import.meta.env?.PUBLIC_SUPABASE_ANON_KEY as string) || "";
      if (!svc || !key) return;
      await fetch(`${svc}/rest/v1/profiles?id=eq.${user.id}`, {
        method: "PATCH",
        headers: {
          apikey: key,
          Authorization: `Bearer ${data?.session?.access_token ?? ""}`,
      ...
      <div className="relative flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-zinc-400 text-xs mt-0.5 leading-relaxed">
            Everything from onboarding is saved. Pick an episode from your
            podcast directory below to run your first pipeline. Azelia processes
