...
export const SettingsForm: React.FC = () => {
  ...
  const PROVIDERS: Record<
    string,
    {
      ...
      ph: string;
      orProvider: "meta" | "openai" | "anthropic" | "google";
      fallbackModels: { id: string; label: string }[];
    }
  > = {
    groq: {
      name: "Groq (Llama)",
      desc: "Extremely fast inference",
      field: "groq_api_key",
      modelField: "groq_model",
      ph: "gsk_...",
      orProvider: "meta" as any,
      fallbackModels: [
    ...
    anthropic: {
      ...
      desc: "Excellent nuance for curation",
      field: "anthropic_api_key",
      modelField: "anthropic_model",
      ph: "sk-ant-...",
      orProvider: "anthropic" as any,
      fallbackModels: [
    ...
    google: {
      name: "Google (Gemini)",
      desc: "Gemini via AI Studio — simple API key",
      field: "google_api_key",
      modelField: "google_model",
      ph: "AIza...",
      orProvider: "google" as any,
      fallbackModels: [
  ...
  const triggerAutoSave = useCallback(async () => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        ...
        if (payload.groq_api_key === "********" || !payload.groq_api_key)
          delete payload.groq_api_key;
        if (payload.openai_api_key === "********" || !payload.openai_api_key)
        ...
        if (payload.google_api_key === "AIza..." || !payload.google_api_key)
          delete payload.google_api_key;
        if (
          payload.transcript_supabase_key === "********" ||
          !payload.transcript_supabase_key
        )
          delete payload.transcript_supabase_key;
      ...
      <form onSubmit={handleSave} className="space-y-8">
              ...
              <div className="p-6 space-y-6">
                <div>
                  ...
                  <input
                    type="text"
                    value={
                      formData.podcast_name ?? settings?.podcast_name ?? ""
                    }
                    onChange={(e) => {
                      setFormData((prev) => ({
                        ...prev,
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={
                        formData.podcast_dir ?? settings?.podcast_dir ?? ""
                      }
                      onChange={(e) => {
                        setFormData((prev) => ({
                          ...prev,
                          podcast_dir: e.target.value,
                        }));
                      }}
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const res = await fetch(
                            `${API_BASE}/resolve-path?name=${encodeURIComponent(folderName)}`,
                            {
                              headers: token
                          ...
                          if (data.count === 1) {
                            setFormData((prev) => ({
                              ...prev,
                              podcast_dir: data.matches[0],
                            }));
                            toast.success(`Found: ${data.matches[0]}`, {
                              id: "resolve",
                            });
                            ...
                            toast.success(
                              `Found ${data.count} matches, using: ${choice}`,
                              { id: "resolve" },
                            );
                          } else {
                            toast.error(
                              `Could not find "${folderName}" on disk. Try the manual picker.`,
                              { id: "resolve" },
                            );
                            setShowBrowser(true);
                          }
                  ...
                  <p className="text-xs text-zinc-500 mt-2">
                    Uses your native file picker. You can also type the full
                    path manually.
                  </p>
            ...
            <section className="bg-zinc-900/40 border border-white/5 rounded-2xl overflow-hidden">
              ...
              <div className="p-4 space-y-2">
                {providerOrder.map((providerId, idx) => {
                  ...
                  // @ts-ignore
                  const hasValue =
                    !!settings?.[config.field] || !!formData[config.field];

                  // Get dynamic models from the native model registry, fallback to hardcoded if loading fails
                  const orGroup = groupedModels.find(
                    (g) => g.provider === config.orProvider,
                  );
                  const dynamicModels =
                    orGroup && orGroup.models.length > 0
                      ? orGroup.models.map((m) => ({
                          id: m.id,
                          label: m.name + (m.is_reasoning ? " 🧠" : ""),
                        }))
                  ...
                  const groupModelsByFamily = (
                    ...
                    for (const m of models) {
                      ...
                      // Vertex / Gemini: gemini-2.5-pro → "Gemini 2.5", gemini-2.0 → "Gemini 2.0"
                        ...
                        familyKey = "Gemini 2.5";
                      } else if (id.includes("gemini-2.0")) {
                        familyKey = "Gemini 2.0";
                      } else if (id.includes("gemini-1.5")) {
                        familyKey = "Gemini 1.5";
                      }
                  ...
                  return (
                    <div
                      key={providerId}
                      className={`border ${isExpanded ? "border-brand-500/50 bg-black/40" : "border-white/5 hover:border-white/10 bg-zinc-900/20"} rounded-xl overflow-hidden transition-all duration-200`}
                    >
                      <div
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
                            <h4
                              className={`font-medium ${isPrimary ? "text-brand-400" : "text-zinc-300"}`}
                            >
                              {config.name}
                            </h4>
                            {isPrimary && (
                              <span className="text-[10px] uppercase tracking-wider bg-brand-500/10 text-brand-400 px-2 py-0.5 rounded border border-brand-500/20">
                                Primary
                              </span>
                            )}
                            {!isPrimary && (
                              <span className="text-[10px] uppercase tracking-wider bg-zinc-800 text-zinc-500 px-2 py-0.5 rounded border border-zinc-700">
                                Fallback {idx}
                              </span>
                            )}
                            {hasValue && (
                        ...
                        <div className="px-4 pb-4 pt-4 border-t border-white/5 bg-black/40 ml-[4.5rem] space-y-4">
                          ...
                          <div>
                            ...
                            <select
                              // @ts-ignore
                              value={
                                formData[config.modelField] ??
                                settings?.[config.modelField] ??
                                dynamicModels[0]?.id
                              }
                              // @ts-ignore
                              onChange={(e) => {
            ...
            <section className="bg-zinc-900/40 border border-white/5 rounded-2xl overflow-hidden mb-6">
              ...
              <div className="p-6">
                <p className="text-sm text-zinc-400 leading-relaxed mb-4">
                  Connect your channel so Azelia can analyze your own past
                  Shorts and feed those patterns to the curation agents —
                  they'll bias clips toward what's working for <em>your</em>{" "}
                  audience. All processing stays local.
                </p>
                <YouTubeConnect />
              </div>
            </section>
            ...
            <section className="bg-zinc-900/40 border border-white/5 rounded-2xl overflow-hidden">
              ...
              <div className="p-6 space-y-6">
                <p className="text-sm text-zinc-400 leading-relaxed -mt-2">
                  If you store your episode transcripts in Supabase, connect
                  your database here so the pipeline can pull from it.
                </p>
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-2">
                  ...
                  <input
                    type="text"
                    value={
                      formData.transcript_supabase_url ??
                    ...
                    onChange={(e) => {
                      setFormData((prev) => ({
                        ...prev,
                        transcript_supabase_url: e.target.value,
                      }));
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setFormData((prev) => ({
                        ...prev,
                        transcript_supabase_key: e.target.value,
                      }));
                    }}
        ...
        <div className="sticky bottom-6 flex justify-end">
          ...
          >
            ...
            {saveStatus === "error" && (
              <>
                <Save className="w-4 h-4" /> Save failed
              </>
            )}
            {saveStatus === "idle" && (
              <>
                <Save className="w-4 h-4 opacity-50" /> Auto-save enabled
              </>
            )}
          </div>
        </div>
