...
const MODEL_LABELS: Record<RetroactiveModel, { short: string; blurb: string }> =
  {
    "claude-haiku-4-5-20251001": {
      short: "Haiku 4.5",
      blurb: "Cheapest. Good enough for classification.",
    },
    "claude-sonnet-4-6": {
      short: "Sonnet 4.6",
      blurb: "Best quality/cost balance.",
    },
    "claude-opus-4-7": {
      short: "Opus 4.7",
      blurb: "Top quality. ~15× Haiku cost.",
    },
  };

export const RetroactiveSyncModal: React.FC<RetroactiveSyncModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const [model, setModel] = useState<RetroactiveModel>(
    "claude-haiku-4-5-20251001",
  );
  ...
  const fetchEstimate = async (chosenModel: RetroactiveModel) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/analytics/youtube/historical/estimate?model=${encodeURIComponent(chosenModel)}`,
        {
          headers: {
      ...
      <div className="relative bg-zinc-950 border border-brand-500/20 shadow-2xl shadow-brand-500/10 rounded-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8 relative">
          ...
          <p className="text-zinc-400 text-sm leading-relaxed mb-6">
            Azelia detected your past YouTube videos. Analyze your previous
            Shorts to immediately understand which <em>hooks</em> and formats
            resonate with your audience. Choose a model based on how you want to
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-black/40 border border-white/5 rounded-xl space-y-3">
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(Object.keys(MODEL_LABELS) as RetroactiveModel[]).map(
                      (m) => {
                ...
                <div className="flex justify-between items-center pt-2 border-t border-white/5">
                  ...
                  <span className="text-lg font-mono text-emerald-400">
                    {estimate?.estimated_cost_usd < 0.01
                      ? "< $0.01"
                      : `$${estimate?.estimated_cost_usd?.toFixed(2)} USD`}
                  </span>
                </div>
              </div>
              ...
              {syncing && (
                <p className="text-xs text-center text-zinc-500 mt-2">
                  This may take a few minutes depending on downloads and the
                  LLM.
                </p>
              )}
            </div>
