    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-zinc-500">
          Run <strong>Extract Creator Signals</strong> above to see what
          patterns work best in your podcast. Your signals never leave your
          cohort.
        </p>
      </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-zinc-900/40 border border-white/5 rounded-2xl p-6">
          ...
          {data.top_hooks && data.top_hooks.length > 0 ? (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs font-mono text-emerald-400">
                      ×{h.performance_premium.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-[11px] text-zinc-500">
                    {h.emotional_charge && (
                      <span className="capitalize">{h.emotional_charge}</span>
                    )}
            ...
            <p className="text-sm text-zinc-500">
              Not enough data yet. Process more shorts.
            </p>
          )}
        </div>
              ...
              {data.topic_performance.map((t) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs font-mono ${t.avg_performance_premium >= 1 ? "text-emerald-400" : "text-zinc-500"}`}
                    >
      ...
      {data.avoid_patterns && data.avoid_patterns.length > 0 && (
        <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-6">
          ...
          <p className="text-xs text-amber-400/70 mb-4">
            These underperformed vs. your own median ({data.signal_count}{" "}
            signals analyzed).
          </p>
            ...
            {data.avoid_patterns.map((a, i) => (
              ...
              >
                <p className="text-sm text-white font-medium capitalize">
                  {(a.hook_type || "generic").replace(/_/g, " ")}
                </p>
                {a.emotional_charge && (
                ...
                <p className="text-[11px] font-mono text-amber-400 mt-1">
                  ×{a.performance_premium.toFixed(2)} · conf{" "}
                  {fmtPct(a.confidence)}
                </p>
              </div>
            ))}
            ...
            {data.retention_patterns.map((r, i) => (
              ...
              >
                ...
                <p className="text-[11px] font-mono text-emerald-400 mt-1">
                  ×{r.performance_premium.toFixed(2)} · conf{" "}
                  {fmtPct(r.confidence)}
                </p>
              </div>
            ))}
