...
export const SyncProgressBubble: React.FC<SyncProgressBubbleProps> = ({
  jobId,
  onComplete,
  onDismiss,
}) => {
    ...
    const pollStatus = async () => {
      try {
        ...
        const res = await fetch(
          `${API_BASE}/analytics/youtube/historical/status/${jobId}`,
          { headers },
        );
        if (!res.ok) return;
          ...
          <div className="px-4 py-3 border-t border-white/5">
            ...
            {isFinished && !isError && (
              <p className="text-xs text-emerald-400/80 mt-1">
                Tu Dashboard de Inteligencia ha sido actualizado con los
                insights de tus videos.
              </p>
            )}
          </div>
        )}
