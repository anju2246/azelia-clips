...
export const RejectedClipsList: React.FC<RejectedClipsListProps> = ({ jobId, onRestored, onClipClick, version = 0 }) => {
...
const RejectedClipCard: React.FC<{ clip: RejectedClip, onRestore: () => void, onClick: () => void }> = ({ clip, onRestore, onClick }) => {
    ...
    return (
        <div className="glass-card rounded-xl overflow-hidden border border-red-500/20 bg-zinc-900/40 flex flex-col opacity-80 hover:opacity-100 transition-opacity">
            <div 
                ...
                onMouseLeave={() => { if (videoRef.current) { videoRef.current.pause(); videoRef.current.currentTime = previewTime; } }}
                onClick={onClick}
            >
                <video 
                    ref={videoRef} 
            ...
            <div className="p-3 flex items-center justify-between bg-zinc-900 gap-3">
                <span className="text-xs text-zinc-400 truncate flex-1" title={clip.filename}>{clip.filename}</span>
                <button 
                    onClick={onRestore}
