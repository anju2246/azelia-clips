...
export const ClipReviewCard: React.FC<ClipReviewCardProps> = ({ clip, jobId, onApprove, onReject, onClick }) => {
    ...
    const handleOpenInFinder = async (e: React.MouseEvent) => {
        e.stopPropagation();
        try { await ClipsApi.openClipLocation(jobId, clip.filename); toast.success('Abierto en Finder'); }
        catch { toast.error('No se pudo abrir'); }
    };

    return (
        <div className={`glass-card rounded-2xl overflow-hidden border ${isReview ? 'border-yellow-500/30' : 'border-white/5'} bg-zinc-900/40 hover:bg-zinc-900/60 transition-all duration-300 flex flex-col`}>
            {/* Video */}
            <div
            ...
            <div className="p-4 flex flex-col flex-1">
                <h3 className="font-semibold text-sm text-zinc-200 mb-1.5 line-clamp-2" title={clip.title}>
                    {clip.title}
                </h3>
                ...
                <div className="flex gap-2 mt-3">
                    {clip.status === 'approved' ? (
                        ...
                        <>
                            <button
                                onClick={(e) => { e.stopPropagation(); onApprove(clip.id); }}
                            ...
                            {showConfirm ? (
                                <div className="flex-1 flex gap-1">
                                    ...
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setShowConfirm(false); onReject(clip.id); }}
