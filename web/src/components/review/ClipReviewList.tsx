...
export const ClipReviewList: React.FC = () => {
  ...
  const [selectedRejectedIndex, setSelectedRejectedIndex] = useState<
    number | null
  >(null);
  ...
  const handleReject = async (clipId: number) => {
    ...
    try {
      ...
      if (
        ...
        selectedClipIndex >= updatedClips.length
      ) {
        setSelectedClipIndex(
          updatedClips.length > 0 ? updatedClips.length - 1 : null,
        );
      }
  ...
  return (
    <div className="w-full">
          ...
          {showTrash ? (
            ...
            <div className="text-center py-20 bg-zinc-900/40 border border-white/5 rounded-2xl">
              <p className="text-zinc-400">
                No clips found for this job. It might have failed or not
          ...
          {loadingHistory ? (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="bg-brand-500/10 text-brand-400 text-xs font-bold px-2 py-1 rounded-md">
                      {item.type === "episode"
                        ? `EP ${item.number.toString().padStart(3, "0")}`
                        : "Ad-Hoc"}
                    </div>
            ...
            <div className="text-center py-20 bg-zinc-900/40 border border-white/5 rounded-2xl max-w-2xl mx-auto mt-10">
              ...
              <p className="text-sm text-zinc-500 mt-2">
                No tienes clips procesados. Ve al Dashboard para procesar un
                video o episodio.
              </p>
              <a
                href="/dashboard"
      ...
      {/* Modal */}
      {(selectedClipIndex !== null || selectedRejectedIndex !== null) &&
        jobId && (
          <ClipPlayerModal
            clips={
              selectedRejectedIndex !== null ? rejectedClips : job?.clips || []
            }
            initialClipIndex={
              selectedRejectedIndex !== null
                ? selectedRejectedIndex
                : selectedClipIndex || 0
            }
            jobId={jobId}
            onClose={() => {
