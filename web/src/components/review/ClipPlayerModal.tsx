...
import { Play, Pause, Volume2, VolumeX, Maximize2, SkipForward, SkipBack, X, FolderOpen, Check, Sparkles, RotateCcw } from 'lucide-react';
...
export const ClipPlayerModal: React.FC<ClipPlayerModalProps> = ({
    clips, initialClipIndex, jobId, onClose, onApprove, onReject
}) => {
    ...
    const isRejected = !currentClip?.status || (currentClip.status !== 'approved' && currentClip.status !== 'review');
    ...
    useEffect(() => {
        ...
        if (videoRef.current) { videoRef.current.load(); videoRef.current.play().catch(() => setIsPlaying(false)); }
    }, [currentIndex]);
    ...
    const handleTimeUpdate = () => {
        ...
        setCurrentTime(cur); setDuration(dur); setProgress((cur / dur) * 100);
    };
    ...
    const seek = (time: number) => {
        ...
        videoRef.current.currentTime = clamped; setCurrentTime(clamped);
    };
    ...
    const toggleMute = () => {
        ...
        if (isMuted && volume === 0) { setVolume(1); videoRef.current.volume = 1; }
    };
    ...
    const handleOpenInFinder = async () => {
        try { await ClipsApi.openClipLocation(jobId, currentClip.filename); toast.success('Abierto en Finder'); }
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 backdrop-blur-xl" onMouseMove={handleMouseMove}>

            {/* Close Button - top right corner */}
            <button onClick={onClose} className="absolute top-5 right-5 z-30 p-2.5 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors backdrop-blur-md border border-white/10">
                <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center justify-center gap-8 h-full max-h-[92vh] px-20">
                ...
                {/* Nav: Previous */}
                <div className={`shrink-0 transition-opacity duration-300 ${currentIndex > 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    <button onClick={handlePrev} className="p-3.5 bg-white/5 hover:bg-brand-600 rounded-full text-white border border-white/10 transition-all hover:scale-110">
                        <SkipBack className="w-5 h-5" />
                    </button>
                </div>

                {/* Video Player */}
                <div className="relative h-full max-h-[85vh] aspect-[9/16] bg-black shadow-2xl rounded-xl overflow-hidden shrink-0" onClick={togglePlay}>
                    <video
                    ...
                    >
                        {/* Progress */}
                        <div className="mb-2">
                            <div className="h-1.5 w-full bg-white/20 rounded-full cursor-pointer relative group overflow-hidden hover:h-2.5 transition-all" onClick={handleProgressScrub}>
                                <div className="absolute top-0 left-0 h-full bg-brand-500 rounded-full pointer-events-none" style={{ width: `${progress}%` }} />
                            </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={toggleFullscreen} className="text-zinc-300 hover:text-white transition-colors ml-auto">
                ...
                {/* Nav: Next */}
                <div className={`shrink-0 transition-opacity duration-300 ${currentIndex < clips.length - 1 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    <button onClick={handleNext} className="p-3.5 bg-white/5 hover:bg-brand-600 rounded-full text-white border border-white/10 transition-all hover:scale-110">
                ...
                <div className="shrink-0 w-72 max-h-[85vh] flex flex-col text-white">
                    ...
                    {currentClip.virality_score !== undefined && (
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-lg font-bold text-white">{currentClip.virality_score}<span className="text-zinc-500 text-sm font-normal">/100</span></span>
                        </div>
                    )}
                    ...
                    {/* Title */}
                    <h2 className="text-lg font-bold leading-tight mb-2">{currentClip.title}</h2>

                    {/* Summary */}
                    <p className="text-sm text-zinc-400 leading-relaxed mb-6">{currentClip.summary}</p>
