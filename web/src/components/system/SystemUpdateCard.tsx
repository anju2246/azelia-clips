...
export function SystemUpdateCard() {
  ...
  const phase =
    status?.status ?? (version?.update_in_progress ? "starting" : "idle");
  const isRunning =
    status?.running ||
    ["starting", "fetching", "installing", "building", "restarting"].includes(
      phase,
    );
    ...
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-5">
        ...
        <div className="mt-4 rounded border border-emerald-900/60 bg-emerald-950/30 p-3">
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs text-emerald-200">
              <span className="font-semibold">v{version.latest}</span> is
              available
      ...
      {confirming && (
        <div className="mt-4 rounded border border-amber-900/60 bg-amber-950/30 p-3 text-xs text-amber-200">
          ...
          <p className="mt-1 text-amber-300/80">
            Active jobs will be paused. Your clips, transcripts and settings
            stay safe (they live outside the install folder).
          </p>
      ...
      {phase === "done" && (
        <div className="mt-4 rounded border border-emerald-900/60 bg-emerald-950/30 p-3 text-xs text-emerald-200">
          ✓ Update complete. The server is restarting; this page will reconnect
          in a few seconds.
        </div>
      )}
    </div>
