  ...
  const handleUpload = async (epNumber: number) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/episodes/${epNumber}/upload-transcript`,
        {
          method: "POST",
        },
      );
    ...
    return (
      <div className="text-center py-12 space-y-4">
        ...
        <p className="text-zinc-500 max-w-sm mx-auto">
          Check your Podcast Directory setting in Settings page. Current path
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {episodes.map((ep) => (
        ...
        >
          ...
          <h3
            className="text-lg font-semibold text-zinc-100 line-clamp-2 mb-2"
            title={ep.title}
          >
            {ep.title}
          </h3>
          ...
          <div className="space-y-2 text-sm text-zinc-500 mb-6">
            <div className="flex items-center gap-2">
              <span
                className={ep.has_video ? "text-emerald-400" : "text-red-400"}
              >
                {ep.has_video ? "✓" : "✗"} Video
              </span>
              ...
              <span
                className={
                  ep.has_transcript ? "text-emerald-400" : "text-yellow-500"
                }
              >
                {ep.has_transcript ? "✓" : "○"} Transcript
              </span>
            </div>
            <p className="text-xs truncate" title={ep.path}>
              {ep.path}
            </p>
          </div>
